...
// average normals
	GLfloat cubo_vertices[] = {
		...
		0.5f, -0.5f,  0.5f,		0.99f,	0.5f,		0.0f,	0.0f,	-1.0f,
		0.5f,  0.5f,  0.5f,		0.99f,	0.75f,		0.0f,	0.0f,	-1.0f,
	...
	Mesh *cubo11 = new Mesh();
	cubo11->CreateMesh(cubo_vertices, cubo_indices, 192, 36);
	meshList.push_back(cubo11);
}
...
int main() 
{
	...
	dadoTexture = Texture("Textures/a1.tga");
	...
	//********carrusel***********************
	T1_a = Model();
	T1_a.LoadModel("Models/06_tapa_1.obj");
	T2_a = Model();
	T2_a.LoadModel("Models/06_tapa_2.obj");
	techo = Model();
	techo.LoadModel("Models/07_techo.obj"); 
	columna = Model();
	columna.LoadModel("Models/14_Columna.obj");
	//****************************************
...
//contador de luces puntuales
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//*********************************************************************
		glm::mat4 model(1.0);			//para guerdar la escala de los objetos
		glm::mat4 modelcentro(1.0);		//para objetos fijos
...
//#################################################################################
	 ...
	 //********************************************
		sp.init();
		sp.load();
		//para posicionarse en el centro del circulo
		modelcentro = model = glm::translate(model, glm::vec3(-1.6f, 0.3f, 0.0f));
		dadoTexture.UseTexture();//FALSE ES PARA QUE NO SEA TRANSPUESTA	
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		sp.render();

		model = glm::rotate(model, glm::radians(gira1* toRadians), glm::vec3(0.0f, 1.0f, 0.0f));
		modelgira = model = glm::translate(model, glm::vec3(1.6f, -0.3f, 0.0f));
	...
	//********************objetos con movimiento ********************
		...
		T1_a.RenderModel();
		//caballo  1d
		modeltemp = model = glm::scale(model, glm::vec3(0.3f, 0.3f, 0.3f));
		...
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));//FALSE ES PARA QUE NO SEA TRANSPUESTA			
		caballo.RenderModel();
		//quitamos la escala, translaciones y rotaciones
		model = modelgira;
		//*******techo***********
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));//FALSE ES PARA QUE NO SEA TRANSPUESTA			
		techo.RenderModel();
		...
		/*sp.init2();
		sp.load();
		model = glm::scale(model, glm::vec3(67.0f, 37.0f, 22.0f));
		model = glm::rotate(model, glm::radians(180.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		dadoTexture.UseTexture();sp.render();
		//*********barco con blender****************
		//model = glm::mat4(1.0);
