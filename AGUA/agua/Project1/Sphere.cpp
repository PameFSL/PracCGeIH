...
Sphere::Sphere(float ratio, int slices, int stacks) :  //inicializador de la esfera
	ratio(ratio), slices(slices), stacks(stacks), VAO(0), VBO(0), EBO(0) {
}

Sphere::~Sphere() {  //destructor de esfera
...
void Sphere::init() {  //Se encarga de generar todos los vertices de la esfera con base a los indices y rebanadas
...
void Sphere::load() { //TODOS LOS DATOS SE LOS PASA A VAO, VBO Y EBO PARA MANDARLOS A LA TARJETA GRÁFICA
	...
	glEnableVertexAttribArray(2);  //SE ENCARGA DE MANEJAR EL COLOR, CADA SUPERFICE PUEDE TENER UN COLOR DIFERENTE, PUEDE FUNCIONAR PARA EL ILUMINADO

	glBindVertexArray(0); // Unbind VAO

}

void Sphere::render() { //DIBUJAR LA ESFERA EN LA PANTALLA
